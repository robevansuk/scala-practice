...
object Server extends StreamApp[IO] {
  ...
  def stream(args: List[String],
    ...
    ServerStream.streamp[IO]
}

object ServerStream {
  import Server._

  def serverRoutes[F[_]: Effect]: HttpService[F] =
    new ServerRoutes[F](serverConfig).service

  def streamp[F[_]: Effect](
      implicit ec: ExecutionContext): fs2.Stream[F, StreamApp.ExitCode] = {
    logger.info(s"Server started on port: ${serverConfig.port.value}")
    BlazeBuilder[F]
      .bindHttp(serverConfig.port.value, "0.0.0.0")
      .mountService(serverRoutes, "/")
      .serve
  }
}
